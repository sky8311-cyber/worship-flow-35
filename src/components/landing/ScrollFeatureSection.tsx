...
export const ScrollFeatureSection = () => {
  ...
  // Phone transformations - reduced max scale on mobile to prevent overlap
  const maxPhoneScale = isMobile ? 1.2 : 1.4;
  const phoneScale = useTransform(scrollYProgress, 
    [0, 0.15, 0.25, 0.55, 0.7], 
    [1, 1, maxPhoneScale, maxPhoneScale, 0.7]
  );
    ...
    <div ref={containerRef} className="relative h-[500vh]">
      ...
      >
        ...
        <div className="relative z-10 flex items-start justify-center pt-8 md:pt-16 min-h-0">
          ...
          {/* CTA Section - centered in Row 1 when visible */}
          {showCTA && (
            <motion.div
        ...
        >
          ...
          {/* Device indicator text - hidden on mobile to prevent overlap */}
          {showDesktop && !showCTA && (
            <motion.p
              initial={{ opacity: 0 }}
              animate={{ opacity: 1 }}
              className="hidden sm:block text-center text-sm text-muted-foreground mt-2"
            >
