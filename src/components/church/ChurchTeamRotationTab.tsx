...
interface ChurchTeamRotationTabProps {
...
export function ChurchTeamRotationTab({ churchAccountId, isAdmin }: ChurchTeamRotationTabProps) {
  ...
  const { data: schedules, isLoading } = useQuery({
    queryKey: ["team-rotation-schedules", churchAccountId],
    queryFn: async () => {
  ...
  // Fetch church members for adding to rotation
  const { data: churchMembers } = useQuery({
    queryKey: ["church-members", churchAccountId],
    queryFn: async () => {
      const { data, error } = await supabase
  ...
  const createScheduleMutation = useMutation({
    ...
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["team-rotation-schedules"] });
          ...
          <div className="flex items-center justify-between">
            <div>
              ...
              <CardDescription>
                {language === "ko" 
                  ? "예배팀 로테이션 스케줄을 관리합니다."
                  : "Manage worship team rotation schedules."}
              </CardDescription>
            </div>
            {isAdmin && (
                  ...
                  <div className="space-y-4 py-4">
                    <div className="space-y-2">
                      ...
                      <Input 
                        ...
                        placeholder="e.g., Main Service Team"
                      />
                    </div>
                    <div className="space-y-2">
                  ...
                  <Select onValueChange={(userId) => addMemberMutation.mutate({ userId })}>
                    ...
                    <SelectContent>
                      {churchMembers?.filter(cm => 
                        !members?.some(m => m.user_id === cm.user_id)
                      ).map((cm) => (
