-- Fix award_seeds function to use correct activity_type names that match triggers
...
BEGIN
  -- Check if this is a one-time activity type (using actual trigger names)
  v_is_one_time := _activity_type IN (
    'signup',
    'first_song_added',      -- trigger uses this, not 'first_song'
    'first_set_created',     -- trigger uses this, not 'first_set'
    'first_community_post',  -- trigger uses this, not 'first_community'
    'profile_setup',         -- trigger uses this, not 'profile_complete'
    'first_team_invite',     -- trigger uses this, not 'first_invite'
    'first_set_published',   -- trigger uses this
    'avatar_upload'          -- one-time avatar upload
  );
  ...
  IF NOT v_is_one_time THEN
    ...
    -- Get daily cap for this activity type (using actual trigger names)
    v_daily_cap := CASE _activity_type
      WHEN 'community_post' THEN 5           -- trigger uses this, not 'post_create'
      WHEN 'song_added' THEN 10              -- trigger uses this, not 'song_add'
      WHEN 'worship_set_created' THEN 5      -- trigger uses this, not 'set_create'
      WHEN 'worship_set_published' THEN 5    -- trigger uses this
      WHEN 'score_uploaded' THEN 10          -- trigger uses this
...
-- Retroactive signup bonus for users who never received it
...
WHERE NOT EXISTS (
  ...
  WHERE st.user_id = p.id AND st.activity_type = 'signup'
);